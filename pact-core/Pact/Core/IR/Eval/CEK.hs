...
evalNormalForm'
  :: CoreCEKEnv -> CoreTerm -> Eval (EvalResult CEKBigStep RawBuiltin () Eval)
evalNormalForm' = evalNormalForm
...
returnCEKValue'
  :: CEKEval step b i m => Cont step b i m -> CEKErrorHandler step b i m -> CEKValue step b i m -> m (CEKEvalResult step b i m)
returnCEKValue' = returnCEKValue

returnCEK'
  :: CEKEval step b i m => Cont step b i m -> CEKErrorHandler step b i m -> EvalResult step b i m -> m (CEKEvalResult step b i m)
returnCEK' = returnCEK

applyLamUnsafe' :: CEKEval step b i m => CanApply step b i m -> [CEKValue step b i m] -> Cont step b i m -> CEKErrorHandler step b i m -> m (EvalResult step b i m)
applyLamUnsafe' = applyLamUnsafe

evalUnsafe'
  :: CEKEval step b i m => CEKEvalResult step b i m -> m (EvalResult step b i m)
...
testCapability :: (IsBuiltin b, CEKEval step b i m, MonadEval b i m) => NativeFunction step  b i m
