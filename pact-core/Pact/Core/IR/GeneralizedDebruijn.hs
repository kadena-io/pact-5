...
data RenamerScope k b =
  RenamerScope
  { _rnDepth :: DeBruijn
  , _rnBinds :: Map k DeBruijn
  , _rnNatives :: Map k b
  }

makeLenses ''RenamerScope

askRenamer :: Applicative m => DeBruijnT k b m (RenamerScope k b)
askRenamer = DeBruijnT (ReaderT pure)

localRenamer :: (RenamerScope k b -> RenamerScope k b) -> DeBruijnT k b m a -> DeBruijnT k b m a
...
debruijnize :: (Ord name, Monad m) => (Text -> name) -> Lisp.Expr name i -> DeBruijnT name b m (Term (Either name DeBruijn) (PactType ParsedTyName) b i)
