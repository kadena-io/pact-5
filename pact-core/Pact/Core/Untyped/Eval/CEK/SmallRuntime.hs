...
module Pact.Core.Untyped.Eval.CEK.SmallRuntime
 ( CEKTLEnv
 , CEKEnv
 , CEKRuntimeEnv(..)
 , BuiltinFn(..)
...
data BuiltinFn b i m
  = BuiltinFn
  { _native :: b
  , _nativeFn :: (MonadEval b i m) => Cont b i m -> CEKErrorHandler b i m -> [CEKValue b i m] -> m (EvalResult b i m, Cont b i m, CEKErrorHandler b i m)
...
mkBuiltinFn
  :: (BuiltinArity b)
  => (Cont b i m -> CEKErrorHandler b i m -> [CEKValue b i m] -> m (EvalResult b i m, Cont b i m, CEKErrorHandler b i m))
  -> b
  -> BuiltinFn b i m
mkBuiltinFn fn b =
