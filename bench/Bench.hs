...
getSize :: (Default i, SizeOf a, IsBuiltin b, Show i) => EvalEnv b i -> EvalState b i -> SizeOfVersion -> a -> IO ()
getSize env state version value = do
  (v, _state) <- liftIO $ runEvalM env state (sizeOf version value)
  return ()
...
main = do
  ...
  coinModule <- expectEval ee es $ liftIO getCoinModule >>= getModule
  defaultMain
    [
      -- 0.003 ms
...
getCoinModule :: IO String
getCoinModule = do
  dataDir <- Paths_pact_tng.getDataDir
  let coinV5Path = dataDir </> "pact-tests" </> "pact-tests" </> "coin-v5" </> "coin-v5.pact"
  readFile coinV5Path
