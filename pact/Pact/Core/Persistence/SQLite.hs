...
withSqlitePactDb serial connectionString act =
  ...
  where
    work db = do
      (pdb, cache) <- liftIO $ initializePactDb serial db
      r <- act pdb
      liftIO $ do
...
mkTblStatement db tbl = do
      ...
      readKeysStmt <-  SQL.prepare db ("SELECT rowkey FROM \""<> tbl <> "\" WHERE rowkey = ? ORDER BY txid")
...
getTxLog serial db currTxId txLog tab txId = do
  ...
  if currTxId' == txId
    ...
    withStmt (SQL.prepare db $ "SELECT rowkey,rowdata FROM \"" <> toUserTable tab <> "\" WHERE txid = ?") $ \stmt -> do
      let TxId i = txId
...
listTxIds db tbl (TxId minTxId) = withStmt (SQL.prepare db $ "SELECT txid FROM \"" <> toUserTable tbl <> "\" WHERE txid >= ? ORDER BY txid ASC") $ \stmt -> do
...
createUserTable serial db txLog stmtCache tbl = do
  ...
  liftIO $ do
    ...
    modifyIORef' stmtCache (\c -> c{_stmtUserTbl = Map.insert tbl stmts (_stmtUserTbl c)})
...
write' serial db txId txLog stmtCache wt domain k v =
  case domain of
    ...
    DNamespaces -> liftIO $ withStmt (_tblInsertOrUpdate . _stmtNamespace <$> readIORef stmtCache) $ \stmt -> do
...
withStmt stmt = bracket stmt SQL.finalize
