...
(expect "- decimal, - decimal decimal" (negate 4.5) (- 3.5 8.0))
(expect "neg integer lit, - integer integer" -2 (- 0 2))
(expect "neg dec lit, - decimal decimal" -4.5 (- 3.5 8.0))
...
(expect-failure "div by zero" (/ 11 0))
(expect-failure "div by zero" (/ 11.0 0.0))
...
;; Todo: these
 (expect "and short-circuit" false
         (and false (error "shouldn't happen")))
 (expect "or short-circuit" true
         (or true (error "shouldn't happen")))
