...
double_literal = float.fromhex('0x1.6a00000000000p+0')
packed = struct.pack('>d', double_literal)
uint64_value = struct.unpack('>Q', packed)[0]
print(f'0x{uint64_value:016x}')

def to_int(v):
  double_literal = float.fromhex(v)
  packed = struct.pack('>d', double_literal)
  return struct.unpack('>Q', packed)[0]

def to_int_with_mul(v, k):
  double_literal = float.fromhex(v) * k
  packed = struct.pack('>d', double_literal)
  return struct.unpack('>Q', packed)[0]

def to_int_with_div(v, k):
  double_literal = float.fromhex(v) / k
  packed = struct.pack('>d', double_literal)
  return struct.unpack('>Q', packed)[0]

def as_hex(v):
  return "0x{0:016x}".format(v)

def to_int_as_hex(v):
  return as_hex(to_int(v))

def to_int_wm_as_hex(v, k):
  return as_hex(to_int_with_mul(v, k))

def to_int_wd_as_hex(v, k):
  return as_hex(to_int_with_div(v, k))


# All the literals in pow_data.c
pow_data_lits = [
...
""".format(
  to_int("0x1.62e42fefa3800p-1"),
  to_int("0x1.ef35793c76730p-45"),
  to_int("-0x1p-1"),
  to_int_with_mul("0x1.555555555556p-2", -2),
  to_int_with_mul("-0x1.0000000000006p-2", -2),
  to_int_with_mul("0x1.999999959554ep-3", 4),
  to_int_with_mul("-0x1.555555529a47ap-3", 4),
  to_int_with_mul("0x1.2495b9b4845e9p-3", -8),
  to_int_with_mul("-0x1.0002b8b263fc3p-3", -8)
  ))

converted_pow_data = [[to_int(x), to_int(y), to_int(z)] for x, y, z in pow_data_lits]
for e in converted_pow_data:
  x, y, z = e
  print("A(0x{0:016x}, 0x{1:016x}, 0x{2:016x})".format(x, y, z))
...
// N/ln2
.invln2N = {{{to_int_wm_as_hex("0x1.71547652b82fep0", 128)}}},
// -ln2/N
.negln2hiN = {{{to_int_as_hex("-0x1.62e42fefa0000p-8")}}},
.negln2loN = {{{to_int_as_hex("-0x1.cf79abc9e3b3ap-47")}}},
// Used for rounding when !TOINT_INTRINSICS
.shift = {{{to_int_as_hex("0x1.8p52")}}},
...
{{{to_int_as_hex("0x1.ffffffffffdbdp-2")}}},
{{{to_int_as_hex("0x1.555555555543cp-3")}}},
{{{to_int_as_hex("0x1.55555cf172b91p-5")}}},
{{{to_int_as_hex("0x1.1111167a4d017p-7")}}},
}},
.exp2_shift = {{{to_int_wd_as_hex("0x1.8p52", 128)}}},
...
{{{to_int_as_hex("0x1.62e42fefa39efp-1")}}},
{{{to_int_as_hex("0x1.ebfbdff82c424p-3")}}},
{{{to_int_as_hex("0x1.c6b08d70cf4b5p-5")}}},
{{{to_int_as_hex("0x1.3b2abd24650ccp-7")}}},
{{{to_int_as_hex("0x1.5d7e09b4e3a84p-10")}}},
}},
""")
